...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	vector< vector <float> > newGrid(height, vector<float>(width,belief_cell));
  ...
  float blurring) 
{
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			...
			float new_value = beliefs[i][j];			
			newGrid[new_i][new_j] = new_value;
		}
	}
